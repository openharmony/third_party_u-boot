...
void vo_drv_set_intf_bt656_cfg(hi_s32 vo_dev, hal_disp_syncinfo *sync_info, hal_disp_syncinv *inv)
{
    const hi_u32 data_mode = 6;
    hi_u32 hd0_div_mode;
    sys_hal_vo_bt_clk_en(HI_TRUE);
...
{
    hi_bool clk_reverse = HI_FALSE;
    /* 2 4 register config */
    const hi_u32 hd0_div_mode = 2;
    const hi_u32 data_mode = 4;
    U_INTF_LCD_CTRL LCD_CTRL;
...
void vo_drv_set_intf_8bit_lcd_cfg(hi_s32 vo_dev, hal_disp_syncinfo *sync_info, vo_intf_sync out_sync)
{
    hi_bool clk_reverse = HI_FALSE;
    /* 3 3 register config */
    const hi_u32 hd0_div_mode = 3;
    const hi_u32 data_mode = 3;
    U_INTF_LCD_CTRL LCD_CTRL;
...
void vo_drv_set_intf_16bit_lcd_cfg(hi_s32 vo_dev, hal_disp_syncinfo *sync_info, vo_intf_sync out_sync)
{
    hi_bool clk_reverse = HI_FALSE;
    /* 2 0 register config */
    const hi_u32 hd0_div_mode = 0;
    const hi_u32 data_mode = 2;
    U_INTF_LCD_CTRL LCD_CTRL;
...
hi_s32 vo_drv_video_set_csc_coef(vo_hal_layer vo_layer, vo_csc_matrix csc_matrix)
{
    ...
    const hi_u32 dc_pre = 4; /* dc pre 4 */
    vo_csc csc = {0};
