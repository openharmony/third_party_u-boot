...
static int update_to_flash(void)
{
	...
	for (i = 0; i < AU_MAXFILES && aufile[i] != NULL; i++) {
...
#ifdef CONFIG_AUTO_OTA_UPDATE
		if (g_is_ota && !is_component_in_ota_header(aufile[i])) {
			debug("%s: not found in header\n", aufile[i]);
			/* maybe only 1 image in package, but 3 images in config */
			continue;
		}
#endif /* CONFIG_AUTO_OTA_UPDATE */
		...
		if (!strcpy(buf, AU_FIRMWARE)) {
			printf("strcpy %s fail\n", aufile[i]);
			continue;
		}
#ifdef CONFIG_AUTO_OTA_UPDATE
		if (g_is_ota &&
			(change_to_ota_path(AU_FIRMWARE, buf,
			sizeof(buf)) != 0)) {
			printf("change ota path fail\n");
			continue;
		}
#endif /* CONFIG_AUTO_OTA_UPDATE */
		if (strcmp(buf, aufile[i]) == 0)
			uboot_updated = 1;
...
{
	char aufile[NAME_MAX_LEN] = AU_CONFIG;
	int ret;
	long sz;

#ifdef CONFIG_AUTO_OTA_UPDATE
	if (g_is_ota &&
		(change_to_ota_path(AU_CONFIG, aufile,
		sizeof(aufile)) != 0)) {
		printf("ota path: not found!\n");
		goto err;
	}
#endif
	sz = file_fat_read(aufile, (void *)LOAD_ADDR, CONFIG_MAX_SIZE);
	if (sz <= 0) {
		printf("ERROR:%s not found!\n", aufile);
		goto err;
	}
...
#ifdef CONFIG_AUTO_OTA_UPDATE
		if (g_is_ota &&
			(change_to_ota_path(aufile_table[i],
			aufile_table[i],
			sizeof(aufile_table[i])) != 0)) {
			printf("change %s fail\n", aufile_table[i]);
			return -1;
		}
#endif
		aufile[i] = &(aufile_table[i][0]);
		printf("[%0d]=%-16s start=0x%08lx end=0x%08lx size=0x%08lx\n",
				i, aufile_table[i],
